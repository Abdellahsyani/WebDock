# WebDock Developer Guide

This guide is for developers who want to extend, debug, or maintain WebDock.
### 1) Development Goals
...
Typical cycle:
    ...
    After changing Dockerfile/compose: make re

    Inspect logs during testing: docker compose logs -f

### 4) How Components Interact

WebDock uses Docker Compose as the orchestrator:
...
Use this order:
    ...
    docker compose config (syntax/merge validation)
    docker compose ps (status)
    docker compose logs -f <service> (runtime errors)
    docker exec -it <container> sh (inside-container checks)
    Rebuild with no cache if required: docker compose build --no-cache <service>
